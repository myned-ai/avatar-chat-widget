...
**Research DONE (3-agent synthesis 2026-07-08) → build spec in
STATE_MACHINE.md §0.6 (sources Part X).** Unanimous conclusion: the continuous
loudness→pitch carrier IS the bobblehead root cause; replace with **ambient
coherent-noise idle (all states) + sparse onset-triggered emphasis beats
(Responding) + state gating**, all spring-smoothed and advanced on the render
clock. Pitch demoted (yaw is the widest real axis); energy SELECTS/SCALES
beats, doesn't drive a carrier. Full signal chain + numbers in §0.6.
Glitch: two suspects — reconstruction stepping (fixed by spring+render-clock)
and idle-clip loop seam (passive probe). BUILD from §0.6 next session.
...
| F12 asset ground truth (offline GLB parse) | **Antonios' pushback ("it worked before, why such a big change") was CORRECT and the F11 fix was built on a false premise.** Offline parse of `animation.glb` (no browser needed): the asset contains **4 clips — idle + 3 speak — each animating ALL 170 bones including head/neckUpper/neckLower** (translation+rotation+scale). Consequences: (1) the mixer resets the head every frame → post-multiply is bounded → **accumulation theory FALSIFIED**, F11 renderer change reverted (src + dist rebuilt); (2) **the "single-clip mode" fix (6e88a30) NEVER ACTIVATED** — it triggers on `animations.length === 1` but the GLB has 4 → the old animation state machine (time=0 snaps, LoopOnce clamps, self-crossfade on 1-clone action lists) has been live the whole time; earlier "fixed" verdicts were the widget-side blend masking it; (3) the SPIN mechanism: the state machine's holes can drive all action weights to ~0 (self-crossfade/finished-clamp) → mixer stops writing the bones → only then do per-frame deltas accumulate; (4) **professional mocap speak clips exist and have been discarded since the 2026-06-11 "locked decision"** — which now looks like a workaround for the broken transitions, and the whole procedural-head effort has been compensating for it. → Strategic fork (Path A clip-first vs Path B procedural) under discussion; craft/industry research re-run in flight. | GLB JSON parse via scratchpad script; `GaussianSplatRenderer.js:1112` passes all 4 clips |
| F11 spinning head (accumulation) | The Exorcist spin when talking. **Caused by my own changes stacking on a fragile renderer contract:** `_applyNeckPose` did `bone.quaternion.multiply(delta)` — bounded ONLY if the active clip re-sets the bone each frame. (a) single-clip mode made every state play the idle clip, which has no head track to reset it; (b) the new HeadMotionController emits a delta EVERY frame in EVERY state (old code returned null in Idle); (c) raised amplitude. ⇒ deltas post-multiplied onto the previous frame without limit → wind-up → spin. **Honest test-gap:** the 22 unit tests prove the CONTROLLER outputs small degrees, but cannot see the RENDERER's application — an integration bug a controller unit test structurally can't catch. **Fix (renderer):** apply delta from a per-bone captured BASE each frame (`bone = base ∘ delta`), non-accumulating by construction; also = §0.6 "procedural owns the head", so it ALSO neutralizes the clip head-track seam (F-glitch). Trade-off: clip's head-track motion is now ignored; with the head-locked rig the body's visible motion comes from procedural (intended). | renderer `_applyNeckPose` post-multiply |
